 ...
 *          ffmpeg -i cat.mp4 -vf "fps=30,scale=-1:320:flags=lanczos,crop=170:in_h:(in_w-170)/2:0" -q:v 7 -preset ultrafast cat_30fps.mjpeg
 *          ffmpeg -i cat.mp4 -ar 44100 -ac 1 -ab 24k -filter:a loudnorm -filter:a "volume=-5dB" 44100.mp3
 */

#define AUDIO_FILENAME "/44100.mp3"
// #define MJPEG_FILENAME "/288_15fps.mjpeg"
#define MJPEG_FILENAME "/chrismas_30fps.mjpeg"
#define FPS 30
// #define MJPEG_BUFFER_SIZE (288 * 240 * 2 / 10)
#define MJPEG_BUFFER_SIZE (320 * 170 * 2 / 10)
...
void setup()
{
  ...
  if (!SD.begin(10 /* CS */, SPI, 80000000))
